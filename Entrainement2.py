...
torch.save(model.state_dict(), path_modele) #Enregistrement du nouveau model
model.load_state_dict(torch.load(path_modele))
model.to(device)
model.eval()
...
for i in range (0,nb_images) :
    ...
    num_gray = clear_border(num_gray)
    num_gray = centrage(num_gray)
    ...
    #plt.show()
    model.float()
    output = model(num_tensor_transform.to(device).float())
